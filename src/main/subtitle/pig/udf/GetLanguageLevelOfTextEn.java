	...
	public Tuple exec(Tuple input) throws IOException {
		...
		if (outputType == OutputType.OUTPUT_BY_SENTENCE || outputType == OutputType.OUTPUT_BY_WORD) {
			if (output.get(0) == null) {
				output.append(BagFactory.getInstance());
			}
		}
		...
		for (CoreMap sentence : sentences) {
			for (CoreLabel token : sentence.get(TokensAnnotation.class)) {
				...
				if (lemma.trim().isEmpty() || punctuation.matcher(lemma).matches() || number.matcher(lemma).matches()) {
					continue;
				}

				Integer levelOfWord = levelByWord.get(lemma);
				if ("PERSON".equals(ne) || "LOCATION".equals(ne) || "NATIONALITY".equals(ne) || "NUMBER".equals(ne)) {
					levelOfWord = 1;
				}
